...
The script attached-herewith will perform all the 5 steps explained in the course project. The script downloads and unzips the dataset and required files read into different variables.

1. Training and test data of X, Y and subject are merged.
2. Columns with  MEAN and STANDARD DEVIATION are extracted from the dataset. Appropriate names are updated from `features.txt` file in the obtained dataset.
...
5. Independent tidy dataset with mean value for each subject and activity is generated. The output file is `tidy_data.txt`.
